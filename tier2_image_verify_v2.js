...
(async () => {
  ...
  const shareRes = await page.evaluate(({ rid, tok, baseUrl }) => fetch(baseUrl + "/api/tier1/share", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: "Bearer " + tok },
    body: JSON.stringify({ reportId: rid }),
  }).then(r => r.json()), { rid: reportId, tok: token, baseUrl: BASE });
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const status = await page.evaluate(({ tid, tok, baseUrl }) => fetch(baseUrl + "/api/tier2/status?tier2Id=" + tid, {
      headers: { Authorization: "Bearer " + tok }
    }).then(r => r.json()), { tid: tier2Id, tok: token, baseUrl: BASE });
