...
(async () => {
  ...
  console.log('=== Local Complete Flow Test ===\n');

  // Register
  const regRes = await fetch(BASE + '/api/auth/login-or-register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ account: '13700000012', password: 'TestPass12', confirmPassword: 'TestPass12' })
  }).then(r => r.json());
  ...
  console.log('Analyze:', JSON.stringify(analyzeRes).substring(0, 200));
  const reportId = analyzeRes.reportId;
  console.log('Report ID:', reportId);

  // Check tier2 BEFORE page load
  const t2Before = await fetch(BASE + '/api/tier2/status?tier1ReportId=' + reportId, {
    headers: { Authorization: 'Bearer ' + token }
  }).then(r => r.json());
  console.log('Tier2 before page:', JSON.stringify(t2Before));
  ...
  await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_complete_start.png', fullPage: true });
  ...
  if (unlockCount > 0) {
    ...
    // Wait for generation
    console.log('\nWaiting for generation (45s max)...');
    try {
      await page.waitForSelector('.report-core-card', { timeout: 45000 });
      console.log('\nSUCCESS: Content appeared!');
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_complete_ready.png', fullPage: true });
    } catch {
      console.log('\nTIMEOUT: Still stuck');
      ...
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_complete_stuck.png', fullPage: true });
    }
  } else {
