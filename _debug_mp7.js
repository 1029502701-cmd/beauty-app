...
async function req(method, path, body, token) {
  return new Promise((res, rej) => {
    ...
    const r = https.request({ hostname: HOST, path: path, method: method, headers: hdrs }, x => {
      ...
      x.on("data", c => b += c);
      x.on("end", () => {
        try { res({ status: x.statusCode, body: JSON.parse(b), headers: x.headers }); }
        catch (e) { res({ status: x.statusCode, body: b.substring(0, 500), headers: x.headers }); }
      });
    });
...
async function main() {
  ...
  console.log("Register:", reg.status, "CF-Ray:", reg.headers["cf-ray"]);
  const token = reg.body.sessionId;
  if (!token) { console.log("No token:", JSON.stringify(reg.body).substring(0, 200)); process.exit(1); }
  console.log("Token:", token);
  
  const j = await req("POST", "/api/tier1/analyze", { reportId: "test" }, token);
  console.log("JSON tier1:", j.status, j.body.reportId ? "OK " + j.body.reportId : JSON.stringify(j.body).substring(0, 200));
  
  const m = await multipartReq("/api/tier1/analyze", "photo", "C:/Users/yao/Documents/ChatGPT/美妆app/photo.jpg", token);
  console.log("Multipart tier1:", m.status, "CF-Ray:", m.headers["cf-ray"], JSON.stringify(m.body).substring(0, 300));
