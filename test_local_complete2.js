const { chromium } = require('playwright');
(async () => {
  const BASE = 'http://localhost:8788';
  console.log('=== Local Complete Flow Test (v2) ===\n');

  // Register
  const regRes = await fetch(BASE + '/api/auth/login-or-register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ account: '13700000013', password: 'TestPass13', confirmPassword: 'TestPass13' })
  }).then(r => r.json());
  const token = regRes.sessionId;
  console.log('Token:', token);

  // Create tiny JPEG
  const jpeg = Buffer.from([
    
  ]);

  const analyzeRes = await fetch(BASE + '/api/tier1/analyze', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + token },
    body: jpeg
  }).then(r => r.json());
  const reportId = analyzeRes.reportId;
  console.log('Report ID:', reportId);

  // Browser test
  const browser = await chromium.launch({ headless: true, args: ['--no-sandbox'] });
  const context = await browser.newContext();
  await context.addInitScript((t) => localStorage.setItem('session_token', t), token);
  const page = await context.newPage();

  const tier2Reqs = [];
  page.on('request', (req) => {
    if (req.url().includes('tier2/status') && req.method() === 'GET') {
      tier2Reqs.push({ t: Date.now(), url: req.url() });
    }
  });
  page.on('pageerror', err => console.log('[JS ERROR]', err.message));

  console.log('\nNavigating to /report?tab=进阶...');
  await page.goto(BASE + '/report?id=' + reportId + '&tab=%E8%BF%9B%E9%98%B6', { waitUntil: 'networkidle', timeout: 20000 });
  await page.waitForTimeout(3000);
  console.log('URL:', page.url().replace(BASE, ''));
  await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc2_start.png', fullPage: true });

  // Check state
  const state = await page.evaluate(() => ({
    processing: document.querySelector('.report-loading p')?.textContent || 'none',
    hasCoreCard: document.querySelector('.report-core-card') !== null,
    allProcessing: Array.from(document.querySelectorAll('.report-loading p')).map(p => p.textContent)
  }));
  console.log('State:', JSON.stringify(state));
  console.log('tier2/status requests:', tier2Reqs.length);

  // Click unlock button
  const unlockCount = await page.locator('button:has-text("看广告解锁")').count();
  console.log('Unlock buttons:', unlockCount);
  
  if (unlockCount > 0) {
    console.log('\n--- Clicking 看广告解锁 ---');
    await page.locator('button:has-text("看广告解锁")').click();
    
    // Wait for loading state
    await page.waitForTimeout(1000);
    const afterClick = await page.evaluate(() => ({
      processing: document.querySelector('.report-loading p')?.textContent || 'none',
      hasCoreCard: document.querySelector('.report-core-card') !== null,
      unlockLoading: document.querySelector('button:has-text("解锁中")') !== null
    }));
    console.log('After click (1s):', JSON.stringify(afterClick));
    console.log('tier2/status requests:', tier2Reqs.length);
    await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc2_after_click.png', fullPage: true });

    // Wait for generation
    console.log('\nWaiting for generation (60s max)...');
    try {
      await page.waitForSelector('.report-core-card', { timeout: 60000 });
      console.log('\nSUCCESS: Content appeared!');
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc2_ready.png', fullPage: true });
      console.log('Total tier2/status requests:', tier2Reqs.length);
      if (tier2Reqs.length > 0) {
        const times = tier2Reqs.map(r => Math.round(r.t - tier2Reqs[0].t));
        console.log('Request intervals (ms):', times.join(', '));
      }
    } catch {
      console.log('\nTIMEOUT: Still stuck in processing');
      console.log('Total tier2/status requests:', tier2Reqs.length);
      if (tier2Reqs.length > 0) {
        const times = tier2Reqs.map(r => Math.round(r.t - tier2Reqs[0].t));
        console.log('Request intervals (ms):', times.join(', '));
      }
      const finalState = await page.evaluate(() => ({
        processing: document.querySelector('.report-loading p')?.textContent || 'none',
        hasCoreCard: document.querySelector('.report-core-card') !== null
      }));
      console.log('Final state:', JSON.stringify(finalState));
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc2_stuck.png', fullPage: true });
    }
  } else {
    console.log('No unlock button found');
    const btns = await page.locator('button').allTextContents();
    console.log('Buttons:', btns.filter(t => t.trim()));
  }

  await browser.close();
  console.log('\n=== Test Complete ===');
})();
