...
(async () => {
  ...
  console.log('=== Local Complete Flow Test (v3) ===\n');

  // Register
  const regRes = await fetch(BASE + '/api/auth/login-or-register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ account: '13700000014', password: 'TestPass14', confirmPassword: 'TestPass14' })
  }).then(r => r.json());
  ...
  await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc3_start.png', fullPage: true });

  // Check state
  const state = await page.evaluate(() => ({
    processingText: Array.from(document.querySelectorAll('.report-loading p')).map(p => p.textContent),
    hasCoreCard: document.querySelector('.report-core-card') !== null,
    allBtns: Array.from(document.querySelectorAll('button')).map(b => b.textContent.trim())
  }));
  ...
  if (unlockCount > 0) {
    ...
    console.log('tier2/status requests:', tier2Reqs.length);
    await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc3_after_click.png', fullPage: true });
    ...
    try {
      ...
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc3_ready.png', fullPage: true });
      ...
      await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\lc3_stuck.png', fullPage: true });
    }
  } else {
    console.log('No unlock button found');
    const btns = await page.locator('button').allTextContents();
    console.log('All buttons:', btns.filter(t => t.trim()));
  }
