...
(async () => {
  ...
  // Use /tier2/status which auto-creates tier2 record and triggers generation
  console.log('\n=== Checking tier2 status (auto-generates) ===');
  ...
  // Poll for completion
  console.log('\n=== Polling tier2 ===');
  const tier2Id = statusRes.tier2ReportId;
  for (let i = 0; i < 40; i++) {
    await page.waitForTimeout(3000);
    const pollRes = await page.evaluate(({ tid, tok }) => fetch('/api/tier2/status?tier2Id=' + tid, {
      headers: { Authorization: 'Bearer ' + tok }
    }).then(r => r.json()), { tid: tier2Id, tok: token });
    console.log('  [' + (i+1) + '] ' + pollRes.generationStatus);
    if (pollRes.generationStatus === 'ready') { console.log('READY!'); break; }
    if (pollRes.generationStatus === 'failed') { console.log('FAILED'); break; }
  }
