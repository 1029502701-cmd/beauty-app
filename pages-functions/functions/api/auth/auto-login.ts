...
export const GET: FrameworkCallbackOptions['GET'] = async (context) => {
  ...
  await env.SESSION_KV.put('session:' + sessionId, JSON.stringify({ userId, gender: url.searchParams.get("gender"), age_range: url.searchParams.get("age_range"), invite_code: url.searchParams.get("invite") || null, expiresAt: now + 7 * 24 * 60 * 60 }), { expirationTtl: 7 * 24 * 60 * 60 });
  return new Response(JSON.stringify({ sessionId, isNew, needPassword, hasPassword: needPassword, inviteCode: url.searchParams.get('invite') || null }), { headers: { 'Content-Type': 'application/json' } });
};
