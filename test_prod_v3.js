...
(async () => {
  ...
  // === TEST 1: Login redirect with fresh browser ===
  ...
  const regRes = await fetch(BASE + '/api/auth/login-or-register', {
    ...
    body: JSON.stringify({ account: '13600000004', password: 'TestPass4', confirmPassword: 'TestPass4' })
  }).then(r => r.json());
  ...
  await page.goto(BASE + '/report?id=44ef8170-9176-40f2-ba2b-7aabb6802ada', { waitUntil: 'networkidle', timeout: 20000 });
  ...
  if (page.url().includes('/login')) {
    ...
    await page.fill('input[placeholder*="手机号 / 邮箱"]', '13600000004');
    await page.fill('input[placeholder*="请输入密码"]', 'TestPass4');
    await page.fill('input[placeholder*="请再次输入密码"]', 'TestPass4');
    ...
    console.log('UNEXPECTED: Not redirected to /login, URL:', page.url().replace(BASE, ''));
    await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_not_login.png', fullPage: true });
  }
  ...
  if (mineRes.reports && mineRes.reports.length > 0) {
    ...
    await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_start.png', fullPage: true });

    // Check tier2 status directly
    const t2Status = await page2.evaluate((tok, rid) => {
      return fetch('/api/tier2/status?tier1ReportId=' + encodeURIComponent(rid), {
    ...
    if (hasUnlockBtn > 0) {
      ...
      if (adVisible > 0) {
        ...
        if (afterAdProcessing > 0 && afterAdContent === 0) {
          ...
          try {
            ...
            await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_ready.png', fullPage: true });
          } catch {
            ...
            await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_stuck.png', fullPage: true });
          }
          ...
          await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_after_ad.png', fullPage: true });
        }
      } else {
        ...
        console.log('State:', JSON.stringify(state));
        await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_no_ad.png', fullPage: true });
      }
    } else if (hasContent > 0) {
      console.log('Content already visible - tier2 already generated');
      await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_already.png', fullPage: true });
    } else {
      ...
      await page2.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\prod_v3_no_btn.png', fullPage: true });
    }

    await browser2.close();
  } else {
    console.log('No reports for this user - need to create one first');
  }
