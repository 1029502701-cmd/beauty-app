...
(async () => {
  ...
  await page.fill('input[placeholder*="手机号"]', '13900000066');
  ...
  const adminRes = await page.evaluate(async () => {
    const res = await fetch('/api/admin/config', {
      ...
      body: JSON.stringify({ key: 'tier2_btn_color', value: '#db2777' })
    });
    return res.json();
  });
  ...
  await page.screenshot({ path: 'test_output/btn_color_after.png', fullPage: true });
  console.log('Screenshot saved');
