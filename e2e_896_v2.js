...
(async () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (status.generationStatus === "ready" && status.content) {
      ...
      console.log("[4] Generated! productRecs: " + Object.keys(status.content.productRecs || {}).join(","));
      break;
    }
    if (i % 5 === 4) console.log("  Waiting... " + (i+1));
  }
  
  if (!tier2Content) { console.log("FATAL: No content"); await browser.close(); process.exit(1); }
  ...
  console.log("[5] Products: " + total + " imgs:" + withImg + " links:" + withLink);
  
  // Browser verification - use session_token as localStorage key
  console.log("\n[6] Browser check...");
  ...
  for (let i = 0; i < Math.min(bulbCount, 6); i++) {
    try {
      ...
      await snap(pg, "e2e_896_b_modal_" + i);
  ...
  await snap(pg, "e2e_896_b_final");
  
  console.log("\n" + "=".repeat(60));
  console.log("  FINAL RESULTS — Deployment 896dde93");
  console.log("  Account: " + NEW_PHONE);
  console.log("  Tier1: " + tier1Id + " | Tier2: " + tier2Id);
  console.log("  API Products: " + total + " | With images: " + withImg + "/" + total + " | With Taobao links: " + withLink + "/" + total);
