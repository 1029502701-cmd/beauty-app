...
async function req(method, path, body, token) {
  return new Promise((res, rej) => {
    ...
    const r = https.request({
      ...
      x.on("data", c => b += c);
      x.on("end", () => {
        try { res({ status: x.statusCode, headers: x.headers, body: JSON.parse(b) }); }
        catch (e) { res({ status: x.statusCode, headers: x.headers, body: b.substring(0, 500) }); }
      });
    });
...
async function multipartReq(path, fieldName, filePath, token) {
  return new Promise((res, rej) => {
    ...
    const r = https.request({
      hostname: "394bd505.beauty-api-pages.pages.dev",
      path: path, method: "POST", headers: hdrs
    }, x => {
      let b = "";
      x.on("data", c => b += c);
      x.on("end", () => {
        try { res({ status: x.statusCode, body: JSON.parse(b) }); }
        catch (e) { res({ status: x.statusCode, body: b.substring(0, 500) }); }
      });
    });
    r.on("error", e => res({ error: e.message }));
...
async function main() {
  ...
  const reg = await req("POST", "/api/auth/register", {
    account: "1" + Date.now().toString().slice(-10),
    password: "PicTest1",
    confirmPassword: "PicTest1"
  });
  ...
  // JSON tier1
  const j = await req("POST", "/api/tier1/analyze", { reportId: "test-json" }, token);
  console.log("JSON tier1:", j.status, JSON.stringify(j.body).substring(0, 200));
