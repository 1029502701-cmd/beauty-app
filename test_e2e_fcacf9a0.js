...
const PROD = "https://fcacf9a0.beauty-api-pages.pages.dev";
...
function apiPost(p, body, token) {
  return new Promise((res, rej) => {
    ...
    const r = https.request({ hostname: "fcacf9a0.beauty-api-pages.pages.dev", path: p, method: "POST", headers: hdrs }, x => { let d = ""; x.on("data", c => d += c); x.on("end", () => { try { res(JSON.parse(d)); } catch(e) { res(d); } }); });
    r.on("error", rej);
...
function apiGet(p, token) {
  return new Promise((res, rej) => {
    const hdrs = {};
    if (token) hdrs["Authorization"] = "Bearer " + token;
    const r = https.request({ hostname: "fcacf9a0.beauty-api-pages.pages.dev", path: p, method: "GET", headers: hdrs }, x => { let d = ""; x.on("data", c => d += c); x.on("end", () => { try { res(JSON.parse(d)); } catch(e) { res(d); } }); });
    r.on("error", rej);
    r.setTimeout(30000, () => { r.destroy(); rej(new Error("timeout")); });
    r.end();
  });
