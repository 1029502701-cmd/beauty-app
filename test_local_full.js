const { chromium } = require('playwright');
(async () => {
  const BASE = 'http://localhost:8788';
  console.log('=== Local Full Flow Test ===\n');

  // Login
  const regRes = await fetch(BASE + '/api/auth/login-or-register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ account: '13700000011', password: 'TestPass11', confirmPassword: 'TestPass11' })
  }).then(r => r.json());
  const token = regRes.sessionId;
  console.log('Token:', token);

  // Create tier1 report
  const jpegHeader = Buffer.from([
    
  ]);
  
  const analyzeRes = await fetch(BASE + '/api/tier1/analyze', {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + token },
    body: jpegHeader
  }).then(r => r.json());
  console.log('Analyze:', JSON.stringify(analyzeRes).substring(0, 300));
  const reportId = analyzeRes.reportId;
  console.log('Report ID:', reportId);

  // Check tier2 status immediately after create
  const t2Before = await fetch(BASE + '/api/tier2/status?tier1ReportId=' + reportId, {
    headers: { Authorization: 'Bearer ' + token }
  }).then(r => r.json());
  console.log('Tier2 before page load:', JSON.stringify(t2Before));

  // Browser test
  const browser = await chromium.launch({ headless: true, args: ['--no-sandbox'] });
  const context = await browser.newContext();
  await context.addInitScript((t) => localStorage.setItem('session_token', t), token);
  const page = await context.newPage();

  const tier2Reqs = [];
  page.on('request', (req) => {
    if (req.url().includes('tier2/status') && req.method() === 'GET') {
      tier2Reqs.push({ t: Date.now(), url: req.url() });
    }
  });

  console.log('\nNavigating to /report?tab=进阶...');
  await page.goto(BASE + '/report?id=' + reportId + '&tab=%E8%BF%9B%E9%98%B6', { waitUntil: 'networkidle', timeout: 20000 });
  await page.waitForTimeout(3000);
  console.log('URL:', page.url());
  await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_full_start.png', fullPage: true });

  // Check state
  const hasUnlockBtn = await page.locator('button:has-text("看广告解锁")').count();
  const hasContent = await page.locator('.report-core-card').count();
  const hasProcessing = await page.locator('.report-loading p').count();
  console.log('Unlock buttons:', hasUnlockBtn, '| Content:', hasContent, '| Processing:', hasProcessing);
  console.log('tier2/status requests:', tier2Reqs.length);

  if (hasUnlockBtn > 0) {
    console.log('\n--- Testing ad unlock ---');
    await page.locator('button:has-text("看广告解锁")').click();
    await page.waitForTimeout(1000);

    // Check if ad shows
    const adVisible = await page.locator('.tier2-ad-overlay').count();
    console.log('Ad overlay visible:', adVisible > 0);
    
    if (adVisible > 0) {
      console.log('Waiting for ad (6s)...');
      await page.waitForTimeout(6000);
      
      const afterAdProc = await page.locator('.report-loading p:has-text("AI 正在生成进阶报告")').count();
      const afterAdContent = await page.locator('.report-core-card').count();
      console.log('After ad - Processing:', afterAdProc, '| Content:', afterAdContent);
      console.log('tier2/status requests:', tier2Reqs.length);
      
      if (afterAdProc > 0 && afterAdContent === 0) {
        console.log('Polling for completion (45s)...');
        try {
          await page.waitForSelector('.report-core-card', { timeout: 45000 });
          console.log('SUCCESS!');
          await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_full_ready.png', fullPage: true });
        } catch {
          console.log('TIMEOUT');
          console.log('Total tier2/status requests:', tier2Reqs.length);
          if (tier2Reqs.length > 0) {
            const times = tier2Reqs.map(r => Math.round(r.t - tier2Reqs[0].t));
            console.log('Intervals (ms):', times.join(', '));
          }
          await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_full_stuck.png', fullPage: true });
        }
      } else if (afterAdContent > 0) {
        console.log('Content already visible!');
        await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_full_ready.png', fullPage: true });
      }
    } else {
      console.log('No ad overlay - checking state');
      const state = await page.evaluate(() => ({
        processing: document.querySelector('.report-loading p')?.textContent || 'none',
        hasCoreCard: document.querySelector('.report-core-card') !== null
      }));
      console.log('State:', JSON.stringify(state));
    }
  } else if (hasContent > 0) {
    console.log('Content already visible!');
    await page.screenshot({ path: 'C:\\Users\\yao\\Documents\\ChatGPT\\美妆app\\local_full_ready.png', fullPage: true });
  } else {
    console.log('No unlock button - checking buttons');
    const btns = await page.locator('button').allTextContents();
    console.log('Buttons:', btns.filter(t => t.trim()).slice(0, 10));
  }

  await browser.close();
  console.log('\n=== Test Complete ===');
})();
