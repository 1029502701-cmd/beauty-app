...
// POST /api/orders/create — 创建订单，返回支付链接（金额从 app_config.tier3_token_price 读取）
export const POST: FrameworkCallbackOptions['POST'] = async (context) => {
  ...
  const payUrl = \/mock-pay.html?orderId=\\;

  return new Response(
    JSON.stringify({ orderId, payUrl, outTradeNo }),
    { headers: { 'Content-Type': 'application/json' } }
  );
};
