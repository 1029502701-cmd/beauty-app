...
(async () => {
  ...
  const phone = "13998966531";
  const password = "TestPass123";
  await page.fill('input[placeholder*="手机"]', phone);
  await page.fill('input[placeholder*="密码"]', password);
  // Wait for button to be enabled
  await page.waitForSelector('button.login-btn:not([disabled])', { timeout: 10000 });
  ...
  await page.goto(BASE + "/capture", { waitUntil: "networkidle", timeout: 15000 });
  ...
  // Trigger tier2 via share endpoint
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (status.generationStatus === "ready" || status.generationStatus === "failed") {
      ...
      if (status.generationStatus === "failed") {
        console.log("  Error:", JSON.stringify(status).substring(0, 200));
      }
      break;
    }
  }
  ...
  const sp = SCREENSHOT_DIR + "/tier2_local_verify_" + Date.now() + ".png";
  ...
  const summary = {
    ...
    allImages: imageStatus.map(img => ({ src: img.src.substring(0, 100), loaded: img.loaded, w: img.naturalWidth, h: img.naturalHeight })),
    screenshotPath: sp,
  };
  console.log("\n=== JSON SUMMARY ===");
