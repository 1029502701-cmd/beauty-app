...
(async () => {
      ...
      if (status.content) {
        ...
        for (const [dim, items] of Object.entries(c.productRecs || {})) {
          for (const item of (items || [])) {
            console.log('  ' + item.name + ' | price:' + item.price + ' | img:' + ((item.imageUrl||'').substring(0,50)) + ' | shop:' + (item.shopTitle||'none'));
          }
        }
      }
      break;
  ...
  await page.screenshot({ path: SCREENSHOT_DIR + '/tier2_deployed_debug.png', fullPage: true });
  ...
  const products = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('.report-product-card')).map((card, i) => {
      ...
      const descEl = card.querySelector('.report-product-desc');
      return {
  ...
  for (const p of products) {
    ...
    if (p.imageUrl) console.log('     img url: ' + p.imageUrl.substring(0, 80));
