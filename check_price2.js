...
db.all('SELECT id, generation_status, content FROM reports_tier2 ORDER BY created_at DESC LIMIT 5', function(e,r) {
  if (r) r.forEach(function(x) {
    try {
      var j = JSON.parse(x.content);
      var hasPR = j.productRecs && Object.keys(j.productRecs).length > 0;
      console.log(x.id, x.generation_status, "hasProductRecs:", hasPR);
      if (hasPR) {
